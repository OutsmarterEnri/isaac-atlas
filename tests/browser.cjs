...
(async()=>{const browser=await chromium.launch({headless:true,...(process.env.ATLAS_BROWSER?{channel:process.env.ATLAS_BROWSER}:{})});try{
 ...
 const wild=page.locator('.live-task').filter({hasText:'Wild Card'});await wild.locator('input').check();await page.waitForTimeout(1800);assert(await wild.locator('input').isChecked());
 await page.locator('#save-slot').selectOption('2');await page.waitForFunction(()=>document.querySelector('#total-unlocked').textContent==='0');assert.equal(await page.locator('.live-task').count(),0);await page.locator('#follow-live').click();await page.waitForFunction(()=>document.querySelector('#live-tasks').textContent.includes('Wild Card'));
 ...
 state={...state,custom:false,run:'second-run'};await page.waitForFunction(()=>document.querySelector('#live-tasks').textContent.includes('Wild Card'));assert.equal(await wild.locator('input').isChecked(),false);
 ...
 await page.locator('#search').fill('Wild Card');await page.locator('#results .card-open').click();assert((await page.locator('#detail').textContent()).includes('Casi d’uso'));await page.keyboard.press('Escape');
