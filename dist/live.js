...
function pickupText(p,data){const name=p.kind==='collectible'?(catalog.find(r=>r.itemId===p.subtype)?.name||`Oggetto #${p.subtype}`):p.kind==='card'?(p.subtype===80?'Wild Card':`Carta #${p.subtype}`):p.kind==='pill'?`Pillola #${p.subtype}`:p.kind==='rune'?`Runa #${p.subtype}`:p.kind==='heart'?`Cuore #${p.subtype}`:p.kind==='bag'?'Sacco':p.kind==='chest'?'Cassa':`${p.kind} #${p.subtype}`;const price=p.price?` · prezzo ${p.price}`:'';return `${name}${price} · posizione ${Math.round(p.x)},${Math.round(p.y)}`}
