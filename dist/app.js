...
function switchSlot(){
 ...
 $('#objective-marks').innerHTML='';$('#results').innerHTML='';$('#character-summary').innerHTML='';$('#empty').hidden=true;$('#more').hidden=true;$('#error').hidden=true;$('#toast').hidden=true;
...
function baseRows(){return catalog.filter(r=>(!$('#hide-unlocked').checked||!unlocked(r))&&(view!=='saved'||saved.has(r.id))&&(view!=='challenges'||r.method==='Sfide'));}
...
function descriptionBlock(r,compact=false){
 const n=noteFor(r);if(!n)return compact?'':`<section class="description-section"><h3>Cosa fa</h3><p class="note-empty">${noteLoadError?escapeHTML(noteLoadError):'Descrizione non disponibile nelle note installate.'}</p><a href="${escapeHTML(r.wiki)}" target="_blank" rel="noopener noreferrer">Leggi l’effetto sulla wiki ↗</a></section>`;
 ...
 return `<section class="description-section"><h3>Cosa fa</h3>${n.nameIt&&n.nameIt!==r.name?`<p class="localized-name">${escapeHTML(n.nameIt)}</p>`:''}<ul lang="${n.language==='it'?'it':'en'}">${n.description.map(line=>`<li>${escapeHTML(line)}</li>`).join('')}</ul><div class="note-sources">${noteLinks([n.descriptionSource])}${n.language!=='it'?'<span>Descrizione disponibile in inglese</span>':''}${n.reviewed?`<span>Verifica documentale: ${escapeHTML(n.reviewed)} · ${escapeHTML(n.scope)}</span>`:''}</div></section>`;
}
...
function strategyBlock(r,compact=false){
 ...
 return `<section class="strategy-section ${compact?'compact':''}"><div class="strategy-heading"><h${compact?'4':'3'}>Casi d’uso e sinergie</h${compact?'4':'3'}>${list.length?`<span>${list.length} note</span>`:''}</div>${list.length?list.map((n,i)=>`<details class="strategy ${n.kind==='caution'?'caution':''}" ${i===0&&compact?'open':''}><summary><span class="strategy-kind">${n.kind==='caution'?'Da sapere':n.kind==='use'?'Caso d’uso':n.kind==='interaction'?'Interazione':'Sinergia'}</span><strong>${escapeHTML(n.title)}</strong></summary><div class="strategy-body" lang="${n.language==='it'?'it':'en'}">${n.body.map(line=>`<p>${escapeHTML(line)}</p>`).join('')}${n.steps?`<ol>${n.steps.map(step=>`<li>${escapeHTML(step)}</li>`).join('')}</ol>`:''}<div class="partners">${(n.partners||[]).map(p=>partnerBadge(p,r)).join('')}</div><div class="note-sources">${noteLinks(n.sources)}${n.reviewed?`<span>Verifica documentale: ${escapeHTML(n.reviewed)} · ${escapeHTML(n.scope)}</span>`:''}</div></div></details>`).join(''):`<p class="note-empty">${noteLoadError?escapeHTML(noteLoadError):'Non ci sono ancora combinazioni verificate in questa scheda.'}</p>`}<a class="wiki-synergies" href="${escapeHTML(r.wiki)}#Synergies" target="_blank" rel="noopener noreferrer">Consulta le interazioni sulla wiki ↗</a></section>`;
}
function card(r){const done=unlocked(r),pin=saved.has(r.id);return `<article class="card ${done?'unlocked':''}"><button class="bookmark ${pin?'saved':''}" data-pin="${r.id}" aria-label="${pin?'Rimuovi':'Aggiungi'} ${escapeHTML(r.name)} ${pin?'da':'a'} Da sbloccare" aria-pressed="${pin}">${bookmarkSvg(pin)}</button><button class="card-open" data-detail="${r.id}" aria-label="Dettagli: ${escapeHTML(r.name)}"><span class="card-top"><span class="item-image reward-art${rewardArtClass(r)}">${AtlasArt.reward(r)}${rewardImage(r)?`<img src="${escapeHTML(rewardImage(r))}" alt="" loading="lazy" width="48" height="48">`:''}</span><span><h3>${escapeHTML(r.name)}</h3><span class="meta">${escapeHTML(r.category)} · Segreto ${String(r.id).padStart(3,'0')}</span></span></span>${descriptionBlock(r,true)}<span class="requirement-label">${done?'SBLOCCATO NEL TUO SAVE':'COME SI SBLOCCA'}</span><span class="requirement">${done?'Già disponibile nel tuo salvataggio.':escapeHTML(translate(r.requirement))}</span></button><div class="card-bottom"><span class="tag">${AtlasArt.svg(r.character?'light':'map')}${escapeHTML(r.character||r.method)}</span><span>${r.boss?AtlasArt.svg(AtlasArt.bosses[r.bosses[0]]):''}${done?'Completato':r.boss?escapeHTML(r.boss):'Da sbloccare'} <span class="card-arrow" aria-hidden="true">↗</span></span></div>${view==='challenges'?challengeBlock(r):''}</article>`;}

function render(){
 if(!progress)return;
 renderObjectiveBoard();
 const rows=visibleRows(),base=baseRows();
 $('#results').classList.toggle('saved-grid',view==='saved');$('#hide-unlocked').disabled=false;$('#visibility-label').textContent='Nascondi sbloccati';$('#saved-explanation').hidden=view!=='saved';
 ...
 if(view==='characters'){
  ...
  $('#character-summary').innerHTML=Object.entries(counts).sort((a,b)=>a[0].localeCompare(b[0])).map(([c,n])=>`<button data-char="${escapeHTML(c)}">${characterIcon(c)}${escapeHTML(c)} <b>${n}</b></button>`).join('');
 }
}
...
let descriptionRefresh=null;
async function refreshDescriptions(){
 if(descriptionRefresh)return descriptionRefresh;
 descriptionRefresh=(async()=>{
  try{
   ...
   notes=data.entries;return true;
  }catch{return false;}finally{descriptionRefresh=null;}
 })();
 return descriptionRefresh;
}
function detail(id,open=true){
 const r=catalog.find(x=>x.id===id);if(!r)return;selected=id;
 if(open&&!noteFor(r))refreshDescriptions().then(loaded=>{if(loaded&&selected===id&&$('#detail').open){detail(id,false);render();}});
 const done=unlocked(r);$('#detail-body').innerHTML=`<p class="eyebrow">SEGRETO ${String(r.id).padStart(3,'0')} · ${escapeHTML(r.category).toUpperCase()}</p><div class="detail-illustration reward-art${rewardArtClass(r)}">${AtlasArt.reward(r)}${rewardImage(r)?`<img src="${escapeHTML(rewardImage(r))}" alt="" width="85" height="85">`:''}</div><h2>${escapeHTML(r.name)}</h2><span class="tag">${done?'Sbloccato nello slot '+slot:'Da sbloccare'}</span>${descriptionBlock(r)}<div class="notice"><strong>${done?'Obiettivo già completato':'Il tuo obiettivo'}</strong><p>${done?'Questo sblocco è già presente nel tuo salvataggio. Non viene proposto nella vista degli oggetti mancanti.':escapeHTML(translate(r.requirement))}</p></div>${!done?`<details><summary>Requisito originale della wiki</summary><p lang="en">${escapeHTML(r.requirement)}</p></details>`:''}${r.character?`<p><strong>Personaggio:</strong> ${escapeHTML(r.character)}</p>`:''}${r.boss?`<p><strong>Obiettivo:</strong> ${escapeHTML(r.boss)}</p>`:''}${challengeBlock(r)}${strategyBlock(r)}${r.character?`<button class="primary" data-plan="${r.id}">Pianifica questo obiettivo</button>`:''}<div class="detail-links"><a href="${escapeHTML(r.wiki)}" target="_blank" rel="noopener noreferrer">Guida completa sulla wiki ↗</a><a href="${escapeHTML(r.fandom)}" target="_blank" rel="noopener noreferrer">Fandom ↗</a></div><button class="primary" data-pin="${r.id}">${saved.has(r.id)?'Rimuovi da Da sbloccare':'Aggiungi a Da sbloccare'}</button>`;
...
document.addEventListener('click',e=>{
 ...
 const nav=e.target.closest('[data-view]');if(nav){navigatePage(nav.dataset.view);}
});
...
$('#refresh').addEventListener('click',()=>{sync();refreshDescriptions().then(loaded=>{if(loaded){render();if(selected&&$('#detail').open)detail(selected,false);}});});
...
document.addEventListener('keydown',e=>{if(['all','saved','characters','challenges'].includes(view)&&e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)&&!$('dialog[open]')){e.preventDefault();$('#search').focus();}});
async function start(){try{await loadLocalArtwork();await discoverSources();const response=await fetch('data/catalog.json');if(!response.ok)throw Error('Catalogo non disponibile');catalog=await response.json();const noteResults=await Promise.allSettled(['notes','strategies'].map(async name=>{const res=await fetch('data/'+name+'.json',{cache:'no-store'});if(!res.ok)throw Error('Note non disponibili');return (await res.json()).entries;}));if(noteResults[0].status==='fulfilled')notes=noteResults[0].value;if(noteResults[1].status==='fulfilled')strategies=noteResults[1].value;if(noteResults.some(r=>r.status==='rejected')){noteLoadError='Una parte delle note non è stata caricata. Ricarica la pagina per riprovare.';toast(noteLoadError);}for(const [id,prop] of [['character','character'],['boss','boss']]){for(const val of [...new Set(catalog.flatMap(r=>prop==='boss'?r.bosses:[r[prop]]).filter(Boolean))].sort()){const opt=document.createElement('option');opt.value=val;opt.textContent=val;$('#'+id).append(opt);}}$('#save-slot').disabled=false;await sync();setInterval(sync,5000);}catch(e){$('#error').hidden=false;$('#error').textContent=e.message;}}
start();
